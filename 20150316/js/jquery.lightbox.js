 ...
 * Copyright (C) 2008 Benjamin Arthur Lupton modify by romy 2015.03.25
...
{	// Create our Plugin function, with $ as the argument (we pass the jQuery object over later)
	...
	{	// Our LightboxClass definition
		...
		speed:			'normal',		// Duration of effect, milliseconds
		...
		domReady: function ( )
		{
			...
			$('body').append('<div id="lightbox-overlay"><div id="lightbox-overlay-text">'+(this.show_linkback?'<p><span id="lightbox-overlay-text-about"><a href="'+this.text.about.text+'" title="'+this.text.about.title+'">'+this.text.about.text+'</a></span></p>':'')+(this.show_helper_text?'<p><span id="lightbox-overlay-text-close">'+this.text.help.close+'</span><span id="lightbox-overlay-text-interact">'+this.text.help.interact+'</span></p>':'')+'</div></div><div id="lightbox"><div id="lightbox-imageBox"><div id="lightbox-imageContainer"><img id="lightbox-image" /><div id="lightbox-nav"><a href="javascript:;" id="lightbox-nav-btnPrev"></a><a href="javascript:;" id="lightbox-nav-btnNext"></a></div><div id="lightbox-loading"><a href="javascript:;" id="lightbox-loading-link"><img src="' + this.files.images.loading + '" /></a></div></div></div><div id="lightbox-infoBox"><div id="lightbox-infoContainer"><div id="lightbox-infoHeader"><span id="lightbox-caption">'+(this.download_link ? '<a href="javascript:;" title="' + this.text.download + '" id="lightbox-caption-title"></a>':'<span id="lightbox-caption-title"></span>')+'<span id="lightbox-caption-seperator"></span><span id="lightbox-caption-description"></span></span></div><div id="lightbox-infoFooter"><span id="lightbox-currentNumber"></span><span id="lightbox-close"><a href="javascript:;" id="lightbox-close-button" title="'+this.text.closeInfo+'">' + this.text.close + '</a></span></div><div id="lightbox-infoContainer-clear"></div></div></div></div>');
			...
			// Assign close clicks
			$('#lightbox-overlay-text-close, #lightbox-btnClose ,#lightbox-close-button').unbind().click(function() {
				$.Lightbox.finish();
				return false;	
			});
		...
		{	// Display the lightbox
			...
			this.repositionBoxes({'speed':'fast'});
		...
		showImage: function ( image, step )
		{
			...
			switch ( step )
			{
				...
				case 4:
					...
					{	// Display
						$('#lightbox-currentNumber').html('<span id="lightbox-menu"></span><span id="lightbox-text">'+this.text.image + ( image.index + 1 ) + this.text.of + this.images.size()+'</span>');
						
				var lightboxMenuHtml="";
				var menuActive="class='menuActive2'";
				for(var i = 0; i < this.images.size(); i++){
					if(image.index == i){
						menuActive="class='menuActive'";
						j=i+2;
						if($('.flipbook').turn("page")){
							$('.flipbook').turn("page", j)
							}else{ 
							return false;
							}
						}else{ menuActive=""}
					lightboxMenuHtml += "<a "+menuActive+" href='javascript:;' onclick='$.Lightbox.showImage("+(i)+")"+"'>"+(i+1)+"</a>";
					}
				$("#lightbox-menu").append(lightboxMenuHtml);
		...
		KeyboardNav_Enable: function ( ) {
			$(document).keydown(this.$keydown=function(objEvent) {
		...
		KeyboardNav_Disable: function ( ) {
			//绑定哪些处理函数，只解绑哪些处理函数
			if(this.$keydown)$(document).unbind('keydown',this.$keydown);
		},
