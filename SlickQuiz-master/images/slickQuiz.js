...
(function($){
    $.slickQuiz = function(element, options) {
        var plugin   = this,
            ...
            defaults = {
                checkAnswerText:  '答题并查看成绩',
                nextQuestionText: '下一题',
                backButtonText: '上一题',
                completeQuizText: '即刻完成答题', //设置为空显示nextQuestionText
                tryAgainText: '重新答题', //设置为空隐藏
                questionCountText: '问答进度 %current/%total',
                preventUnansweredText: '您必须选择至少一个答案',
                questionTemplateText:  '%count. %text',
                scoreTemplateText: '%score/%total',
                nameTemplateText:  '<span>测验：</span>%name',
                skipStartButton: false,
                numberOfQuestions: null,
                randomSortQuestions: false,
                randomSortAnswers: false,
                preventUnanswered: true,  //是否必选
                disableScore: false,
                selectedScore: true,
            ...
            _selectedScore         = _element + ' .selectedScore',
        ...
        // selectedScore Count
		if (plugin.config.selectedScore){
			var jsonscore=0;
			var selectescore=0;
		}
        ...
        plugin.method = {
            ...
            setupQuiz: function(options) { // use 'options' object to pass args
                ...
                if (plugin.config.tryAgainText && plugin.config.tryAgainText !== '') {
                    $quizResultsCopy.append('<p><a class="button ' + tryAgainClass + '" href="javascript:;">' + plugin.config.tryAgainText + '</a></p>');
                }
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        if (plugin.config.backButtonText && plugin.config.backButtonText !== '') {
                            questionHTML.append('<a href="javascript:;" class="button ' + backToQuestionClass + '">' + plugin.config.backButtonText + '</a>');
                        }
                        ...
                        if (!plugin.config.perQuestionResponseMessaging) {
                            questionHTML.append('<a href="javascript:;" class="button ' + nextQuestionClass + ' ' + checkAnswerClass + '">' + nextText + '</a>');
                        } else {
                            questionHTML.append('<a href="javascript:;" class="button ' + nextQuestionClass + '">' + nextText + '</a>');
                            questionHTML.append('<a href="javascript:;" class="button ' + checkAnswerClass + '">' + plugin.config.checkAnswerText + '</a>');
                        }
            ...
            checkAnswer: function(checkButton, options) {
                ...
                var selectedAnswers = [],checkscore = 0;
                ...
                answerSelects.each( function() {
					...
					// get the score
					if (plugin.config.selectedScore){
						selectescore=quizJSON.questions[id.replace(/(.*\_question)/, '').replace(/(_.*)/, '')].a[parseInt(id.replace(/(.*\_question\d{1,}_)/, ''), 10)].score;
						checkscore += parseInt(selectescore, 10);
					}
					
                });
				
				if (plugin.config.selectedScore){
					jsonscore+=checkscore;
					//alert(jsonscore);
				}
            ...
            completeQuiz: function(options) {
                ...
                if (plugin.config.selectedScore) {
                    $(_selectedScore + ' span').html(jsonscore);
                } else {
                    $(_selectedScore).remove();
                }
